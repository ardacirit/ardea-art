...
/** Strip + trim any text to a meta-description-safe length (word boundary) */
export function metaDescription(text, fallback = '') {
  ...
  return `${cut.slice(0, lastSpace > 80 ? lastSpace : 155).trimEnd()}…`
}

/**
 * Site title with a keyword-bearing descriptor. Respects a custom descriptor
 * the editor may already have typed (detected by a dash/pipe in the value).
 */
export function siteTitleFor(settings, lang) {
  const descriptor =
    lang === 'en' ? 'Turkish Çini & Ceramic Artist' : 'Çini ve Seramik Sanatçısı'
  const base = settings?.siteTitle?.trim()
  if (!base) return `${ARTIST_NAME} — ${descriptor}`
  return /[—–|-]/.test(base) ? base : `${base} — ${descriptor}`
}
