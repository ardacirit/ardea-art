...
export default function Navbar() {
  ...
  const textColor = onHero ? 'text-ardea-text' : 'text-ardea-text'
    ...
    <>
      <header
        className={`fixed top-0 inset-x-0 z-40 transition-all duration-500
          ${scrolled
            ? 'bg-ardea-white/96 backdrop-blur-md shadow-sm py-3'
            : 'bg-ardea-white/0 py-5'
          }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <Link to="/" className="flex flex-col leading-none group">
            <span className={`font-serif text-xl tracking-wide transition-colors duration-200 group-hover:text-ardea-cobalt ${textColor}`}>
              Zerrin Cirit
            </span>
      ...
      >
        ...
        <button
          ...
          className="absolute top-5 right-6 text-ardea-text"
        ...
        <nav className="flex flex-col items-center gap-8">
          {links.map(({ to, label }, i) => (
            <NavLink
              ...
              onClick={() => setMenuOpen(false)}
              className={({ isActive }) =>
                `font-serif text-3xl text-ardea-text transition-all duration-300 hover:text-ardea-cobalt
